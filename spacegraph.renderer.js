...
const renderPeriodMS = 1000/15; //30fps
...
Renderer.prototype.resizeToFit = function(renderer){

    //console.log('resize', this.element.getBoundingClientRect());



    const ele = renderer.view;
    ...
    //console.log(ele, w, h);
    //ele.setAtttribute('width', w);
    //ele.setAtttribute('height', h);
    renderer.resize(w, h);
...
Renderer.prototype.setupGUI = function() {
    ...
    // console.log($(this.renderer.view).parent());
    // $(this.renderer.view).parent().append(this.gui.domElement)
    // console.log(gui);
    //this.renderer.view.parent.appendChild(gui);
...
Renderer.prototype.updateGUI = function() {
    ...
    function updateControllers(folder){
        // First level
        folder.__controllers.forEach(function (i) {
            let c = folder.__controllers[i];
            if (c) {
                console.log(c.updateDisplay);
...
Renderer.prototype.startRenderingLoop = function(){
    ...
    function update(time){
        ...
        if (deltaTime >= renderPeriodMS) {
            lastTime = time;
            demo.render(deltaTime/1000);
        }
...
WebGLRenderer.prototype.init = function(){
    ...
    this.renderer =     PIXI.autoDetectRenderer(s.width, s.height, {
    ...
    window.onresize = function(){
        that.resizeToFit(that.renderer);
    };
    ...
    this.setCameraCenter([0, 0]);
    setTimeout(()=>{
        this.resizeToFit(this.renderer);
    },0);
...
WebGLRenderer.prototype.shapeDrawFunctions[p2.Shape.CONVEX] =
    WebGLRenderer.prototype.drawConvex = function(g, shape, color, alpha, lineWidth){
        ...
        var vrot = shape._drawConvexVec2; //cache for this vec2
        if (!vrot) {
            vrot = shape._drawConvexVec2 = vec2.create();
        }
